...
void DeviceManagerWidget::UpDateWidget()
{
	...
	QList<DevicDataManager::ProtStruct> protList=m_deviceDataManager->m_deviceList.value(m_oldDevName).protList;
	...
	QStringList strList=ui.treeWidget->currentItem()->text(0).split(" ");
		for(int i=0;i<protCount;i++)
		{
			if(m_deviceDataManager->m_deviceList.value(ui.treeWidget->topLevelItem(0)->text(0))
				.protList.at(i).protocolName.c_str()==ui.treeWidget->currentItem()->parent()->text(0))
			{
				...
				{
					if(m_deviceDataManager->m_deviceList.value(ui.treeWidget->topLevelItem(0)->text(0))
						.protList.at(i).messgeList.at(j).messageName.c_str()==strList.at(0)
						&&ui.treeWidget->currentItem()->parent()->child(j)==ui.treeWidget->currentItem())
					{
						...
						.protList[i].messgeList[j].messageP= QString::fromLocal8Bit("发布").toStdString();
	...
	for(int i=0;i<protCount;i++)
	{
			...
			.protList.at(i).protocolName.c_str()==ui.treeWidget->currentItem()->parent()->text(0))
		{
			...
			for(int j=0;j<msgCount;j++)
			{
				...
				{
					m_deviceDataManager->m_deviceList[ui.treeWidget->topLevelItem(0)->text(0)]
					.protList[i].messgeList[j].messageP= QString::fromLocal8Bit("订阅").toStdString();
					m_deviceDataManager->UpdateAll();
					return;
				}
			}
