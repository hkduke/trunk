...
bool QtEnvTest::on_slot_Device(QPoint point)
{
	...
	QString dataString = ui.m_treeWidget_devManager->currentItem()->data(0, Qt::UserRole).toString();
	if (dataString=="protocolName")
	{
		m_actin_saveSignal->setEnabled(false);
	}
	else
	{
		m_actin_saveSignal->setEnabled(true);
	}
	m_popMenu->exec(QCursor::pos());
	return true;
}
...
void QtEnvTest::setDeviceList()
{
	...
	for(int i=0;i<m_deviceDataManager->m_devNameList.size();i++)
	{
		...
		for(int j=0;j<m_deviceDataManager->m_deviceList[devName].protList.size();j++)
		{
			QTreeWidgetItem *thirdItem=new QTreeWidgetItem(secondItem);
			QTreeWidgetItem* fourthItem = new QTreeWidgetItem(thirdItem);
			thirdItem->setText(0, m_deviceDataManager->m_deviceList[devName].protList.at(j).port.c_str());
			thirdItem->setData(0, Qt::UserRole, "port");
			fourthItem->setText(0,m_deviceDataManager->m_deviceList[devName].protList.at(j).protocolName.c_str());
			fourthItem->setData(0,Qt::UserRole,"protocolName");
		}
	}
		qDebug()<<"envset"<<m_deviceDataManager->m_deviceList.keys();
}
//显示更新报文列表
...
void QtEnvTest::on_deleteDevice()
{
	...
	if(rowNumber!=-1)
	{
		...
		{
			m_deviceDataManager->m_deviceList.take(deviceName);
			m_deviceDataManager->m_devNameList.removeAll(deviceName);
			setDeviceList();
			if(deviceManager->GetCurrentDevName()==deviceName)
			{
				deviceManager->Clear();
			}
		}
		else if(currentItem->data(0,Qt::UserRole).toString()=="protocolName")
		{
			...
			for(int i=0;i<m_deviceDataManager->m_deviceList.value(parentItem->text(0)).protList.size();i++)
			{
				DevicDataManager::ProtStruct protStruct=m_deviceDataManager->m_deviceList.value(parentItem->text(0))
					.protList.at(i);
				if(QString(protStruct.protocolName.c_str())==deviceName)
				{
					int index=m_deviceDataManager->m_deviceList[parentItem->text(0)].protList.indexOf(protStruct);
					m_deviceDataManager->m_deviceList[parentItem->text(0)].protList.takeAt(index);
				}
			}
