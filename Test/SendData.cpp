...
>>>>>>> .r289
			...
			/*	PackManager* pack = new PackManager;
				pack->Fill(11200);
				DataWriteHandle handel2;
				handel2.HandleEnter(pack);
*/
...
=======
				...
				//int stdi = 0;
 				for (int i = 0; i < vecpack.size(); i++)
 				{
					...
					//stdi += pack1.GetLenth();
					//LogManager::GetInstance().WriteMessage("SendData::run 发送的包序号为%d，%d",(int)pack->GetFileSerial(data), len);
 				}
