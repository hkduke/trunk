...
namespace NetCom
{
	...
	void ServiceHandler::Send(BYTE * data, UInt32 len)
	{
		Timestamp stemp;
...
// 			//std::cout << AnyCast<int>(pack.GetPropValue(pack.m_messageID)) << std::endl;
			...
			LogManager::GetInstance().WriteMessage("ServiceHandler::onReadable：msgType:%d",i);
	...
	bool TCPTransport::Init()
	{
		...
		for (ResourceManager::NodeList::iterator iterList = list.begin();iterList!= list.end(); iterList++,++index)
		{
			...
			if (iterMap == m_connectstatus.end())
			{
				...
				link->SetDataControlInfo(DataLink::SubPub);

				link->SetTransport(this);
				link->SetDscNodeId((*iterList)->GetNodeID());
				...
				Framework::GetInstance().GetAckQueueManager()->RegsiterDataLinkFun(link, nullptr);
