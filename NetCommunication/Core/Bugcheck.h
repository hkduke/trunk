...
#define poco_bugcheck_msg(msg)          do { std::abort(); } while (0)
	...
	#define poco_assert_dbg(cond) \
		if (!(cond)) NetCom::Bugcheck::assertion(#cond, __FILE__, __LINE__); else (void) 0

	#define poco_assert_msg_dbg(cond, text) \
		if (!(cond)) NetCom::Bugcheck::assertion(#cond, __FILE__, __LINE__, text); else (void) 0
#else
	#define poco_assert_msg_dbg(cond, text)
	#define poco_assert_dbg(cond)
#endif
...
#define poco_check_ptr(ptr) \
	if (!(ptr)) NetCom::Bugcheck::nullPointer(#ptr, __FILE__, __LINE__); else (void) 0


#define poco_bugcheck() \
	NetCom::Bugcheck::bugcheck(__FILE__, __LINE__)


#define poco_bugcheck_msg(msg) \
	NetCom::Bugcheck::bugcheck(msg, __FILE__, __LINE__)
...
#define poco_unexpected() \
	NetCom::Bugcheck::unexpected(__FILE__, __LINE__);


#define poco_debugger() \
	NetCom::Bugcheck::debugger(__FILE__, __LINE__)


#define poco_debugger_msg(msg) \
	NetCom::Bugcheck::debugger(msg, __FILE__, __LINE__)
