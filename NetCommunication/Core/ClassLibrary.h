...
extern "C"
{
	bool POCO_LIBRARY_API pocoBuildManifest(NetCom::ManifestBase* pManifest);
...
extern "C"	\
{			\
	bool POCO_LIBRARY_API POCO_JOIN(pocoBuildManifest, name)(NetCom::ManifestBase* pManifest); \
}
...
#define POCO_BEGIN_MANIFEST_IMPL(fnName, base) \
	bool fnName(NetCom::ManifestBase* pManifest_)										\
	{																				\
		typedef base _Base;															\
		typedef NetCom::Manifest<_Base> _Manifest;									\
		std::string requiredType(typeid(_Manifest).name());							\
		std::string actualType(pManifest_->className());							\
		if (requiredType == actualType)												\
		{																			\
			NetCom::Manifest<_Base>* pManifest = static_cast<_Manifest*>(pManifest_);
...
#define POCO_EXPORT_CLASS(cls) \
    pManifest->insert(new NetCom::MetaObject<cls, _Base>(#cls));


#define POCO_EXPORT_SINGLETON(cls) \
	pManifest->insert(new NetCom::MetaSingleton<cls, _Base>(#cls));
