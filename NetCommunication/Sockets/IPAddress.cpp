...
using NetCom::NumberFormatter;
using NetCom::BinaryReader;
using NetCom::BinaryWriter;
using NetCom::toLower;
using NetCom::trim;
using NetCom::UInt8;
using NetCom::UInt16;
using NetCom::UInt32;
using NetCom::IPAddressImpl;
using NetCom::IPv4AddressImpl;
#if defined(POCO_HAVE_IPv6)
using NetCom::IPv6AddressImpl;
#endif
...
IPAddress::IPAddress(Family family)
{
	if (family == IPv4)
		...
		newIPv6();
#endif
	else throw NetCom::InvalidArgumentException("Invalid or unsupported address family passed to IPAddress()");
}
	...
	: _pImpl(0)
{
	if (length == sizeof(struct in_addr))
		...
		newIPv6(addr);
#endif
	else throw NetCom::InvalidArgumentException("Invalid address length passed to IPAddress()");
}


IPAddress::IPAddress(const void* addr, poco_socklen_t length, NetCom::UInt32 scope)
{
	if (length == sizeof(struct in_addr))
		newIPv4(addr);
#if defined(POCO_HAVE_IPv6)
...
IPAddress::IPAddress(unsigned prefix, Family family)
{
	if (family == IPv4)
	{
		if (prefix <= 32)
			...
			throw NetCom::InvalidArgumentException("Invalid prefix length passed to IPAddress()");
	}
#if defined(POCO_HAVE_IPv6)
	else if (family == IPv6)
	{
...
IPAddress& IPAddress::operator = (const IPAddress& addr)
{
	if (&addr != this)
	{
		if (addr.family() == IPAddress::IPv4)
			...
			throw NetCom::InvalidArgumentException("Invalid or unsupported address family");
	}
	return *this;
}
...
NetCom::UInt32 IPAddress::scope() const
{
	return pImpl()->scope();
}
...
IPAddress IPAddress::broadcast()
{
 ...
 NetCom::BinaryWriter& operator << (NetCom::BinaryWriter& writer, const NetCom::IPAddress& value)
{
	writer << static_cast<NetCom::UInt8>(value.length());
	writer.writeRaw(reinterpret_cast<const char*>(value.addr()), value.length());
	return writer;
}


 NetCom::BinaryReader& operator >> (NetCom::BinaryReader& reader, NetCom::IPAddress& value)
{
	char buf[sizeof(struct in6_addr)];
	NetCom::UInt8 length;
	reader >> length;
	reader.readRaw(buf, length);
	value = NetCom::IPAddress(buf, length);
	return reader;
}


std::ostream& operator << (std::ostream& ostr, const NetCom::IPAddress& addr)
{
	ostr << addr.toString();
	return ostr;
}
