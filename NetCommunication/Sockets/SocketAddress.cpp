...
using NetCom::RefCountedObject;
using NetCom::NumberParser;
using NetCom::UInt16;
using NetCom::InvalidArgumentException;
using NetCom::SocketAddressImpl;
using NetCom::IPv4SocketAddressImpl;
#ifdef POCO_HAVE_IPv6
using NetCom::IPv6SocketAddressImpl;
#endif
#ifdef POCO_OS_FAMILY_UNIX
using NetCom::Impl::LocalSocketAddressImpl;
#endif
...
SocketAddress::SocketAddress(const IPAddress& hostAddress, NetCom::UInt16 portNumber)
{
	init(hostAddress, portNumber);
}


SocketAddress::SocketAddress(NetCom::UInt16 portNumber)
{
	init(IPAddress(), portNumber);
}


SocketAddress::SocketAddress(Family fam, NetCom::UInt16 portNumber)
{
	init(IPAddress(fam), portNumber);
}


SocketAddress::SocketAddress(const std::string& hostAddress, NetCom::UInt16 portNumber)
{
	init(hostAddress, portNumber);
}


SocketAddress::SocketAddress(Family fam, const std::string& hostAddress, NetCom::UInt16 portNumber)
{
...
#endif
	else throw NetCom::InvalidArgumentException("Invalid address length or family passed to SocketAddress()");
}
...
NetCom::UInt16 SocketAddress::port() const
{
...
void SocketAddress::init(const IPAddress& hostAddress, NetCom::UInt16 portNumber)
{
...
#endif
	else throw NetCom::NotImplementedException("unsupported IP address family");
}


void SocketAddress::init(const std::string& hostAddress, NetCom::UInt16 portNumber)
{
	IPAddress ip;
...
void SocketAddress::init(Family fam, const std::string& hostAddress, NetCom::UInt16 portNumber)
{
	IPAddress ip;
	if (IPAddress::tryParse(hostAddress, ip))
	{
...
NetCom::UInt16 SocketAddress::resolveService(const std::string& service)
{
	unsigned port;
 ...
 NetCom::BinaryWriter& operator << (NetCom::BinaryWriter& writer, const NetCom::SocketAddress& value)
{
	writer << value.host();
	writer << value.port();
	return writer;
...
NetCom::BinaryReader& operator >> (NetCom::BinaryReader& reader, NetCom::SocketAddress& value)
{
	NetCom::IPAddress host;
	reader >> host;
	NetCom::UInt16 port;
	reader >> port;
	value = NetCom::SocketAddress(host, port);
	return reader;
}


std::ostream& operator << (std::ostream& ostr, const NetCom::SocketAddress& address)
{
