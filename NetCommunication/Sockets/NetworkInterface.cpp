...
using NetCom::FastMutex;
using NetCom::format;


std::ostream& operator << (std::ostream& os, const NetCom::NetworkInterface::MACAddress& mac)
{
	std::ios state(0);
	state.copyfmt(os);
	for (unsigned i = 0; i < mac.size(); ++i)
	{
		if (i > 0) os << NetCom::NetworkInterface::MAC_SEPARATOR;
...
class NetworkInterfaceImpl: public NetCom::RefCountedObject
{
public:
...
inline const IPAddress& NetworkInterfaceImpl::address(unsigned index) const
{
	...
	else throw NotFoundException(NetCom::format("No address with index %u.", index));
}
...
const IPAddress& NetworkInterfaceImpl::subnetMask(unsigned index) const
{
	...
	throw NotFoundException(NetCom::format("No subnet mask with index %u.", index));
}
...
const IPAddress& NetworkInterfaceImpl::destAddress(unsigned index) const
{
	...
	throw NotFoundException(NetCom::format("No address with index %u.", index));
}
...
IPAddress subnetMaskForInterface(const std::string& name, bool isLoopback)
{
	...
	else
	{
		...
		std::wstring usubKey;
		NetCom::UnicodeConverter::toUTF16(subKey, usubKey);
		...
		NetCom::UnicodeConverter::toUTF8(unetmask, netmask);
#else
...
NetworkInterface::Map NetworkInterface::map(bool ipOnly, bool upOnly)
{
	...
	NetCom::Buffer<UCHAR> memory(outBufLen);
	...
	for (; pAddress; pAddress = pAddress->Next)
	{
		...
		NetCom::UnicodeConverter::toUTF8(pAddress->FriendlyName, name);
		NetCom::UnicodeConverter::toUTF8(pAddress->Description, displayName);
#else
