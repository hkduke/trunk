...
class Foundation_API SocketAddress
...
public:
	...
	SocketAddress(const IPAddress& hostAddress, UInt16 portNumber);
		/// Creates a SocketAddress from an IP address and given port number.

	explicit SocketAddress(UInt16 port);
		/// Creates a SocketAddress with unspecified (wildcard) IP address
		/// and given port number.

	SocketAddress(Family family, UInt16 port);
		/// Creates a SocketAddress with unspecified (wildcard) IP address
		/// of the given family, and given port number.

	SocketAddress(const std::string& hostAddress, UInt16 portNumber);
		/// Creates a SocketAddress from an IP address and given port number.
		///
		/// The IP address must either be a domain name, or it must
		/// be in dotted decimal (IPv4) or hex string (IPv6) format.

	SocketAddress(Family family, const std::string& hostAddress, UInt16 portNumber);
		/// Creates a SocketAddress from an IP address and given port number.
		///
		/// The IP address must either be a domain name, or it must
		/// be in dotted decimal (IPv4) or hex string (IPv6) format.
...
protected:
	void init(const IPAddress& hostAddress, UInt16 portNumber);
	void init(const std::string& hostAddress, UInt16 portNumber);
	void init(Family family, const std::string& hostAddress,UInt16 portNumber);
...
private:
	...
	typedef AutoPtr<Impl> Ptr;

	Ptr pImpl() const;

	void newIPv4();
	void newIPv4(const sockaddr_in*);
	void newIPv4(const IPAddress& hostAddress, UInt16 portNumber);

#if defined(POCO_HAVE_IPv6)
	void newIPv6(const sockaddr_in6*);
	void newIPv6(const IPAddress& hostAddress, UInt16 portNumber);
#endif
...
{
	_pImpl = new IPv4SocketAddressImpl(sockAddr);
}


inline void SocketAddress::newIPv4(const IPAddress& hostAddress,UInt16 portNumber)
{
	_pImpl = new IPv4SocketAddressImpl(hostAddress.addr(), htons(portNumber));
}
...
{
	_pImpl = new IPv6SocketAddressImpl(sockAddr);
}


inline void SocketAddress::newIPv6(const IPAddress& hostAddress, UInt16 portNumber)
{
	_pImpl = new IPv6SocketAddressImpl(hostAddress.addr(), htons(portNumber), hostAddress.scope());
}
...
Foundation_API NetCom::BinaryWriter& operator << (NetCom::BinaryWriter& writer, const NetCom::SocketAddress& value);
Foundation_API NetCom::BinaryReader& operator >> (NetCom::BinaryReader& reader, NetCom::SocketAddress& value);
Foundation_API std::ostream& operator << (std::ostream& ostr, const NetCom::SocketAddress& address);
