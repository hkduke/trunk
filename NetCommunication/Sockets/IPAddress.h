...
class Foundation_API IPAddress
...
public:
	...
	IPAddress(const void* addr, poco_socklen_t length, UInt32 scope);
		/// Creates an IPAddress from a native internet address.
		/// A pointer to a in_addr or a in6_addr structure may be 
...
private:
	...
	void newIPv6(const void* hostAddr, UInt32 scope);
...
{
	_pImpl = new IPv4AddressImpl(hostAddr);
}
...
{
	_pImpl = new IPv6AddressImpl(hostAddr);
}


inline void IPAddress::newIPv6(const void* hostAddr, UInt32 scope)
{
	_pImpl = new IPv6AddressImpl(hostAddr, scope);
}
...
Foundation_API NetCom::BinaryWriter& operator << (NetCom::BinaryWriter& writer, const NetCom::IPAddress& value);
Foundation_API NetCom::BinaryReader& operator >> (NetCom::BinaryReader& reader, NetCom::IPAddress& value);
Foundation_API std::ostream& operator << (std::ostream& ostr, const NetCom::IPAddress& addr);
