...
public:
	...
	{
		_pReactor->addEventHandler(_socket, Observer<SocketAcceptor,
		...
		/// Destroys the SocketAcceptor.
	{
		try
		{
			if (_pReactor)
			{
				_pReactor->removeEventHandler(_socket, Observer<SocketAcceptor,
					ReadableNotification>(*this, &SocketAcceptor::onAccept));
			}
		}
		catch (...)
	...
	{
		_pReactor = &reactor;
		if (!_pReactor->hasEventHandler(_socket, Observer<SocketAcceptor,
			ReadableNotification>(*this, &SocketAcceptor::onAccept)))
		{
		...
		/// the reactor.
	{
		...
		_pReactor = &reactor;
		_pReactor->addEventHandler(_socket, Observer<SocketAcceptor, ReadableNotification>(*this, &SocketAcceptor::onAccept));
	}
		...
		/// implementation or directly unregister the accept handler.
	{
		if (_pReactor)
		{
			_pReactor->removeEventHandler(_socket, Observer<SocketAcceptor, ReadableNotification>(*this, &SocketAcceptor::onAccept));
		}
	}
