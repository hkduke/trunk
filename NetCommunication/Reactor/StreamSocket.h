...
class Foundation_API StreamSocket: public Socket
...
public:
	...
	void connect(const SocketAddress& address, const NetCom::Timespan& timeout);
	...
	int sendBytes(NetCom::FIFOBuffer& buffer);
		/// Sends the contents of the given buffer through
	...
	int receiveBytes(NetCom::FIFOBuffer& buffer);
		/// Receives data from the socket and stores it
