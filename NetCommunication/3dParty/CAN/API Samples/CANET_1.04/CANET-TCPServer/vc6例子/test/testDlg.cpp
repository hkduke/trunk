...
#define  IDT_REFRESH_CLIENT	 0X301
/////////////////////////////////////////////////////////////////////////////
...
void CTestDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_REMOTE_CLIENT, m_ctlListBoxClient);
	...
	DDX_Text(pDX, IDC_EDIT_SER_PORT, m_iServerPort);
	//}}AFX_DATA_MAP
}
...
void CTestDlg::OnButtonStart() 
{
	...
	UpdateData(TRUE);
	m_devind = m_ComboIndex.GetCurSel();
	if (m_iServerPort == 0) 
	{
		MessageBox("端口号不能为0!", "警告");
		return;
	}
	...
	int iType = TCP_SERVER;
	//设置服务器或是客户端
	VCI_SetReference(m_devtype, m_devind, 0, CMD_TCP_TYPE, (PVOID)&iType );
	VCI_SetReference(m_devtype, m_devind, 0, CMD_SRCPORT, (PVOID)&m_iServerPort);
	...
	//更新客户连接数目
	SetTimer(IDT_REFRESH_CLIENT, 1000, NULL);
}
...
void CTestDlg::OnButtonDisconn() 
{
	// TODO: Add your control notification handler code here
	int i = m_ctlListBoxClient.GetCurSel();
	VCI_SetReference(m_devtype, m_devind, 0, CMD_DISCONN_CLINET, &(m_Client) );
}
...
void CTestDlg::OnTimer(UINT nIDEvent) 
{
	...
	if (nIDEvent == IDT_REFRESH_CLIENT){
		
		int isel = m_ctlListBoxClient.GetCurSel();
		m_ctlListBoxClient.ResetContent();

		int iClinetCount = 0;
		VCI_GetReference(m_devtype, m_devind, 0, CMD_CLIENT_COUNT, (PVOID)&iClinetCount);
		if (iClinetCount > 0){


			CString strTmp;
			for (int i=0; i<iClinetCount; i++){	
				m_Client.iIndex = i;
				VCI_GetReference(m_devtype, m_devind, 0, CMD_CLIENTS, (PVOID)&m_Client);
				strTmp.Format("%s:%d", m_Client.szip, m_Client.port);
				m_ctlListBoxClient.InsertString(i, strTmp);
			}

			if (isel < iClinetCount){
				m_ctlListBoxClient.SetCurSel(isel);
			}
		}
		
		//VCI_GetReference(m_devtype, index, 0, CMD_CLIENT_COUNT, (PVOID)&m_iClientType );
	}
