...
void __fastcall TForm1::Button1Click(TObject *Sender)
{
        ...
        int cannum =0;
        int srcport=StrToInt(Edit7->Text);
        char szcaneteip[50];
        int caneteport=StrToInt(Edit3->Text);
        strcpy(szcaneteip,Edit2->Text.c_str());

        if(index>=0)
        {
                if(VCI_OpenDevice(m_devtype,index,srcport)==1)
                {
                       VCI_SetReference(m_devtype,index,0,0,(void*)szcaneteip);
                       VCI_SetReference(m_devtype,index,0,1,(void*)&caneteport);
                       Button1->Caption ="断开";
                       m_connect=1;
                       m_devind=index;
                       m_cannum=cannum;
