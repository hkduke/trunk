...
// 	m_packManager->RegisterFunction(&NodeDiscoverTest::recv,11200);
// 	m_packManager->Fill(11200);
	
	Framework::GetInstance().RegisterRecvFunction(&NodeDiscoverTest::recv, 11200);

	m_packManager = (Framework::GetInstance().GetResourceManager()->GetPackManager());
 ...
 void NodeDiscoverTest::recv(BasePackage* parement,UInt32 topicid)
{
	 static int cnt = 0;
	 //ui.tbRecv->clear();
	 //获取所有字段
 ...
 void NodeDiscoverTest::on_btnSend_clicked()
 {
	 ...
	 int ret = Framework::GetInstance().GetTransportStratagy()->Send(111,data, length);
	 if (ret == -1)
	 {
 ...
 void NodeDiscoverTest::on_btnSendFile_clicked()
 {
	 ...
	 std::vector<FilePack*> vecpack = FilePack::GetFilePackages((void*)array.constData(), file.size(), file.fileName().toStdString().c_str(), 0);
	 int stdi = 0;
	 for (int i = 0; i < vecpack.size(); i++)
	 {
		 ...
		 Framework::GetInstance().GetTransportStratagy()->Send(111,(UINT8*)data, len, TransactionType_File);
		 //FilePack pack1;
